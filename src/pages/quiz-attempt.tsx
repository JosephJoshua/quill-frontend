...
export default function QuizAttemptPage() {
  ...
  if (isError) return <div className="p-8 text-destructive">Failed to load quiz results.</div>;
  ...
  return (
    <div className="container max-w-3xl mx-auto p-4 sm:p-8">
      ...
      <Card className="mb-8"><CardHeader><CardTitle>{t('quiz.score')}</CardTitle></CardHeader><CardContent><div className="text-5xl font-bold text-center mb-4">{attempt.score}%</div><Progress value={attempt.score} /></CardContent></Card>
      <div className="space-y-4">
        {attempt.answers.map((ans, index) => (
          <div key={ans.id} className="p-4 border rounded-lg">
            <div className="flex justify-between items-center"><p className="font-semibold">Question {index + 1}</p>
              {ans.isCorrect ? (<span className="flex items-center text-green-600"><CheckCircle className="h-4 w-4 mr-1" /> {t('quiz.correct')}</span>) : (<span className="flex items-center text-red-600"><XCircle className="h-4 w-4 mr-1" /> {t('quiz.incorrect')}</span>)}
            </div>
      ...
      <div className="text-center mt-8"><Button asChild><Link to="/library">{t('quiz.backToLibrary')}</Link></Button></div>
    </div>
  );
}
