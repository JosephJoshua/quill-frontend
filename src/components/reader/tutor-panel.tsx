...
export function TutorPanel({ contentId }: TutorPanelProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.slice(0, -1).concat({ userMessage, aiResponse: res.response })
      );
    } catch (error) {
            ...
            <div key={index} className="space-y-4">
              <div className="flex items-start gap-3 justify-end">
                <div className="flex-1 space-y-1 text-right">
                  ...
                  <div className="bg-primary text-primary-foreground p-3 rounded-lg inline-block text-left">
                    {msg.userMessage}
                  </div>
                </div>
              </div>
